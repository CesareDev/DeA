...
void BigDamagePotion::UpdateAttackZone(UpdateArgs args, float dt)
{
	...
	{
		m_ElapsedTime += dt;
		if (m_DamageArea.getRadius() < 32.f)
			m_DamageArea.setRadius(m_DamageArea.getRadius() + 32.f * dt);
		else
			m_DamageArea.setRadius(0.f);
...
void BigDamagePotion::Attack(float dt)
{
	...
	if (m_IsAttacking)
	{
		...
		{
			if (std::abs(m_AttackAngle) < 90.f)
				m_Angle = 90.f * sinf(2 * acos(-1.f) * (m_ElapsedTime - 1.25f)) - 90.f + m_AttackAngle;
			else
				m_Angle = -90.f * sinf(2 * acos(-1.f) * (m_ElapsedTime - 1.25f)) + 90.f + m_AttackAngle;
			setRotation(m_Angle);
		}
